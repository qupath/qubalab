...
class ImageFeature(geojson.Feature):
    ...
    def __init__(
        ...
        extra_geometries: Optional[dict[str, geojson.geometry.Geometry]] = None,
        ...
        if measurements is not None:
            props["measurements"] = ImageFeature._remove_NaN_values_from_measurements(
                measurements
            )
        ...
        if extra_geometries is not None:
            props["extra_geometries"] = {
                k: add_plane_to_geometry(v) for k, v in extra_geometries.items()
            }
    ...
    def create_from_feature(cls, feature: geojson.Feature) -> ImageFeature:
        ...
        if plane is not None:
            geometry = add_plane_to_geometry(
                geometry, z=getattr(plane, "z", None), t=getattr(plane, "t", None)
            )
        ...
        object_type = next(
            (
                o
                for o in ObjectType
                if o.name.lower() == str(object_type_property).lower()
            ),
            None,
        )
        ...
        if nucleus_geometry is not None:
            if plane is not None:
                nucleus_geometry = add_plane_to_geometry(
                    nucleus_geometry,
                    z=getattr(plane, "z", None),
                    t=getattr(plane, "t", None),
                )
    ...
    def create_from_label_image(
        ...
        for geometry, label in rasterio.features.shapes(
            input_image, mask=mask, connectivity=connectivity, transform=transform
        ):
            if label in existing_features:
                existing_features[label]["geometry"] = shapely.geometry.shape(
                    geometry
                ).union(shapely.geometry.shape(existing_features[label]["geometry"]))
            else:
                if isinstance(classification_names, str):
                    classification_name = classification_names
                elif (
                    isinstance(classification_names, dict)
                    and int(label) in classification_names
                ):
    ...
    def object_type(self) -> ObjectType:
        ...
        return next(
            (
                o
                for o in ObjectType
                if o.name.lower() == str(self.properties["object_type"]).lower()
            ),
            None,
        )
    ...
    def __setattr__(self, name, value):
        if name == "classification":
            ...
            self.properties[
                "measurements"
            ] = ImageFeature._remove_NaN_values_from_measurements(value)
            ...
            self.properties["extra_geometries"][
                _NUCLEUS_GEOMETRY_KEY
            ] = add_plane_to_geometry(value)
        else:
            super().__setattr__(name, value)

    @staticmethod
    def _remove_NaN_values_from_measurements(
    ...
    @staticmethod
    def _to_id_string(object_id: Optional[Union[int, str, uuid.UUID]]) -> str:
    ...
    def _validate_rgb_value(value: Union[int, float]) -> int:
        ...
        if isinstance(value, int):
            if value >= 0 and value <= 255:
                return value
        raise ValueError(
            "Color value must be an int between 0 and 255, or a float between 0 and 1"
        )
