...
class BioIOServer(ImageServer):
    ...
    def __init__(
        ...
        path: str,
        scene: int = 0,
        detect_resolutions=True,
        bioio_kwargs: dict[str, Any] = {},
        **kwargs
    ):
        """
    ...
    def level_to_dask(
        ...
        if level < 0 or level >= self.metadata.n_resolutions:
            raise ValueError(
                "The provided level ({0}) is outside the valid range ([0, {1}])".format(
                    level, self.metadata.n_resolutions - 1
                )
            )

        axes = (
            ("T" if self.metadata.n_timepoints > 1 else "")
            + (
                ("S" if "S" in self._reader.dims.order else "C")
    ...
    def _build_metadata(self) -> ImageMetadata:
        return ImageMetadata(
            ...
            self._get_shapes(self._reader, self._scene)
            if self._detect_resolutions
            else (
                self._get_scene_shape(self._reader, self._reader.scenes[self._scene]),
            ),
    ...
    def _read_block(self, level: int, region: Region2D) -> np.ndarray:
        ...
        return self._reader.get_image_dask_data(axes)[
            t, z, :, y : y + height, x : x + width
        ].compute()

    @staticmethod
    def _get_shapes(reader: BioImage, scene: int) -> tuple[ImageShape, ...]:
        shapes = []
        for _scene in reader.scenes[scene:]:
            shape = BioIOServer._get_scene_shape(reader, _scene)
    ...
    @staticmethod
    def _get_scene_shape(reader: BioImage, scene: Union[int, str]) -> ImageShape:
        ...
        return ImageShape(
            ...
            c=reader.dims.S
            if "S" in reader.dims.order
            else (reader.dims.C if "C" in reader.dims.order else 1),
    ...
    def _is_rgb(reader: BioImage, scene: int) -> bool:
        reader.set_scene(scene)
        return ("S" in reader.dims.order and reader.dims.S in [3, 4]) or (
            reader.dtype == np.uint8 and reader.dims.C == 3
        )
    ...
    def _is_lower_resolution(base_shape: ImageShape, series_shape: ImageShape) -> bool:
        ...
        """
        if (
            base_shape.z == series_shape.z
            and base_shape.t == series_shape.t
            and base_shape.c == series_shape.c
        ):
