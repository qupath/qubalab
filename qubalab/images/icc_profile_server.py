...
class IccProfileServer(WrappedImageServer):
    ...
    def __init__(
        ...
        icc_profile: Optional[
            Union[str, bytes, ImageCms.ImageCmsProfile, ImageCms.ImageCmsTransform]
        ] = None,
        **kwargs,
    ):
        """
        ...
        if icc_profile is None:
            icc_profile = IccProfileServer._get_icc_bytes_from_path(
                base_server.metadata.path
            )
        ...
        if icc_profile is None:
            ...
            self._icc = ImageCms.buildTransformFromOpenProfiles(
                icc_profile, ImageCms.createProfile("sRGB"), "RGB", "RGB"
            )

        if self._icc is None:
            warnings.warn(
                f"No ICC Profile found for {base_server.metadata.path}. Returning original pixels."
            )

    @property
    def icc_transform(self) -> Union[ImageCms.ImageCmsTransform, None]:
        """
            ...
            return np.transpose(
                np.array(
                    ImageCms.applyTransform(
                        Image.fromarray(np.transpose(image, axes=[2, 1, 0])), self._icc
                    )
                ),
                axes=[2, 1, 0],
            )
    ...
    @staticmethod
    def _get_icc_bytes_from_path(path) -> Union[bytes, None]:
        try:
            with tifffile.TiffFile(path) as tif:
                if 34675 in tif.pages[0].tags:
                    return io.BytesIO(tif.pages[0].tags[34675].value).getvalue()
                else:
                    return None
