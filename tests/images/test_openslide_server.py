...
def test_number_of_channels_when_alpha_not_stripped():
    openslide_server = OpenSlideServer(
        multi_resolution_uint8_3channels.get_path(), strip_alpha=False
    )
...
def test_number_of_channels_when_single_channel():
    openslide_server = OpenSlideServer(
        multi_resolution_uint8_3channels.get_path(), single_channel=True
    )
...
def test_read_full_resolution_image():
    ...
    expected_pixels = np.array(
        [
            [
                [
                    multi_resolution_uint8_3channels.get_pixel_value(
                        downsample, x, y, c
                    )
...
def test_recursion():
    ...
    downsamples = [1, 2, 4]
    images = openslide_server.to_dask(downsamples)
    assert all(
        a == b
        for a, b in zip(
            [image.shape[2] for image in images],
            [full_resolution.x / i for i in downsamples],
        )
    )
